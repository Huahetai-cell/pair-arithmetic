...
此外，使用 `tools/io_validation.py` 对实际 `Myapp.exe` 进行了13个黑盒输入输出场景检查，覆盖帮助信息、缺失/非法参数、模式冲突、文件缺失、最小范围、100题、10000题、UTF-8 BOM、带分数、批改矩阵及覆盖写。脚本会重新解析全部生成题，逐题核对精确答案和规范判重键。全部场景通过。最终博客已直接嵌入完整检验表格，不依赖附件展示。
